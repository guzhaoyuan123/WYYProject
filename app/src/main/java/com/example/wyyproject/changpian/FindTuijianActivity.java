...
public class FindTuijianActivity extends AppCompatActivity implements View.OnClickListener {
...
//        Glide.with(this).load(picture).into(discsmap);
    ...
    private void initView(int position) {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    String json = Http.get("http://10.0.2.2:3000/song/url?id=" + FindTuiJianCommon.musicList.get(position).getId() + "");
                    Log.e("????????????", "" + json);
    ...
    private void play(String path, long length,int position) {
        imgMusicGeMinga.setText(FindTuiJianCommon.musicList.get(position).getName());
        imgMusicZuoZhe.setText(FindTuiJianCommon.musicList.get(position).getAr().get(0).getName());
        Glide.with(this).load(FindTuiJianCommon.musicList.get(position).getAl().getPicUrl()).into(discsmap);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.img_music_fanhui:
                FindTuijianActivity.this.finish();
                break;
            case R.id.listen_back_img:
                position--;
                if (position == -1) {
                    position = FindTuiJianCommon.musicList.size()-1;
                }
                initView(position);
            ...
            case R.id.listen_next_img:
                position++;
                if (position == FindTuiJianCommon.musicList.size()) {
                    position = 0;
                }
                initView(position);
                pauseIv.setImageResource(R.mipmap.starplay);
