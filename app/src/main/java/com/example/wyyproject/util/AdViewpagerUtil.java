...
import com.example.wyyproject.adapter.ImageViewPagerAdapter;
...
public class AdViewpagerUtil {
    ...
    private void initAdimgs(String[] urls)
    {
        ...
        for (int i = 0; i <length; i++) {
            ...
            imageView.setBackgroundResource(R.drawable.image_find_banner);
        ...
        if (urls.length > 0) {
            ...
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    ...
                    Glide.with(context).load(urls[i]).apply(options).into(mImageViews[i + 1]);
